...
class CustomDataset(Dataset):
    def __init__(self, image_paths, labels, transform=None):
        ...
        Args:
            ...
            transform (callable, optional): An optional function/transformation to be applied to the images.
        """
...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='cifar10',
                        choices=['cifar10', 'cifar100', 'food101', 'miniImagenet'], help='dataset')
...
def set_loader(opt):   
    ...
    if opt.dataset in ['cifar100', 'cifar10', 'miniImagenet', 'food101']:
        # TRAIN #
        # Load data from .npy
        tmp_data = np.load(opt.npy_file_train, allow_pickle=True)
        ...
        # TEST #
        # Load data from .npy
        tmp_data = np.load(opt.npy_file_test, allow_pickle=True)
        paths = [elem[0] for elem in tmp_data]
        labels = [elem[1] for elem in tmp_data]
        # Convert labels to integer
        label_encoder = LabelEncoder()
        labels_int = label_encoder.fit_transform(labels)
        # Load dataset with custom loader
        val_dataset = CustomDataset(image_paths=paths,
                                    labels=labels_int,
